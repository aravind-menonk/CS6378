...
public class Client {
    ...
    private int max, min, factor; // Configurations to control waiting time before entering CS
    ...
    public void enterCriticalSection() throws InterruptedException{
        for(int i = 0; i < 20; i++){
            try{
                int waitingTime = new Random().nextInt((max - min) + 1) + min;
                Thread.sleep((factor * waitingTime));
